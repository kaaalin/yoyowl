...
function LoginScreen({ onAuthenticated }: { onAuthenticated: (username: string) => void }) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      onAuthenticated(username.trim().toLowerCase())
    } catch {
    ...
    <main className="login-shell">
      <section className="login-card" aria-labelledby="login-title">
        <div className="login-brand-row">
          <img src="/worldline-logo.png" alt="Worldline" className="login-worldline-logo" />
          <span className="login-brand-divider" aria-hidden="true" />
          <div className="login-genesys-lockup">
        ...
        <div className="login-copy">
          <span className="login-eyebrow">Secure campaign workspace</span>
          <h1 id="login-title">Sign in to YoYo</h1>
          <p>Enter your approved account details to access the merchant migration campaign.</p>
        </div>
        ...
        <p className="login-footnote">Access is limited to authorized Worldline campaign users.</p>
      </section>
    </main>
  )
}
...
export default function Home() {
  ...
  if (authState === "signed-out") {
    return <LoginScreen onAuthenticated={(authenticatedUser) => {
      setUsername(authenticatedUser)
      setAuthState("signed-in")
    }} />
  }

  return <YoYoApplication username={username} onLogout={handleLogout} />
}

function YoYoApplication({ username, onLogout }: { username: string; onLogout: () => void }) {
  ...
  const userInitials = username.startsWith("peter.") ? "PL" : "KY"
